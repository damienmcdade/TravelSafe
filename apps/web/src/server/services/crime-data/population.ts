...
// Single source of truth for the per-city population denominator used
// across the app. Imported by safety-score.ts, the AI assistant tool
// definitions, and the National Average card. Keeping one map prevents
// the drift we saw when three copies existed (each one labeled the
// vintage differently from the values it actually held).
//
...
//
// DO NOT ADD CITIES HERE without first adding them to CITIES in
// ./cities.ts — every city in this map must have a working adapter.
