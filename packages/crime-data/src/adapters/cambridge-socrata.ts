...
let inFlightCambridgeFetch: Promise<Incident[]> | null = null;
export async function getRowsCambridge(): Promise<Incident[]> {
  const now = Date.now();
  if (cache && cache.rows.length > 0 && now - cache.fetchedAt < CACHE_TTL_MS) return cache.rows;
  if (inFlightCambridgeFetch) return inFlightCambridgeFetch;
  inFlightCambridgeFetch = (async () => {
    try {
      ...
      inFlightCambridgeFetch = null;
    }
  })();
  return inFlightCambridgeFetch;
}
