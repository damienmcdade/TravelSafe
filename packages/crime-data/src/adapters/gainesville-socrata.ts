...
let inFlightGainesvilleFetch: Promise<Incident[]> | null = null;
export async function getRowsGainesville(): Promise<Incident[]> {
  const now = Date.now();
  if (cache && cache.rows.length > 0 && now - cache.fetchedAt < CACHE_TTL_MS) return cache.rows;
  if (inFlightGainesvilleFetch) return inFlightGainesvilleFetch;
  inFlightGainesvilleFetch = (async () => {
    try {
      ...
      inFlightGainesvilleFetch = null;
    }
  })();
  return inFlightGainesvilleFetch;
}
