...
async function fetchPage(offset: number): Promise<MplsRow[]> {
  ...
  const body = await readJson(res) as { features?: Array<{ attributes: MplsRow }>; error?: unknown };
  ...
  if (body.error) throw new Error(`Minneapolis ArcGIS body error offset=${offset}`);
