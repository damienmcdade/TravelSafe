...
async function fetchPage(offset: number): Promise<IndyRow[]> {
  ...
  const body = await readJson(res) as { features?: Array<{ attributes: IndyRow }>; error?: unknown };
  ...
  if (body.error) throw new Error(`Indianapolis ArcGIS body error offset=${offset}`);
