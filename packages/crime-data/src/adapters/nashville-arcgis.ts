...
async function fetchPage(offset: number): Promise<MnpdRow[]> {
  ...
  const body = (await readJson(res)) as { features?: Array<{ attributes: MnpdRow }>; error?: unknown };
  ...
  if (body.error) throw new Error(`Nashville ArcGIS body error offset=${offset}`);
