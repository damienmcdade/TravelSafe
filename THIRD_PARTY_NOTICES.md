...
boundary geometry is OSM-derived. The same OSM-derived-boundary pattern is used
for Nashville, Houston, Long Beach, Gainesville, and Milwaukee neighborhood
polygons; the notice above applies equally to those.

## US Census Bureau TIGER/Line — public domain (Maryland county place boundaries)

Montgomery County, MD and Prince George's County, MD are county jurisdictions
whose "neighborhoods" are their constituent municipalities and Census Designated
Places (Silver Spring, Rockville, Bethesda… / Bowie, College Park, Hyattsville…).
Those place polygons
(`packages/crime-data/src/data/montgomery-county-neighborhoods.ts`,
`prince-georges-county-neighborhoods.ts`, generated by
`tools/build-md-county-neighborhoods.mjs`) are derived from US Census Bureau
TIGER/Line via the public TIGERweb service. TIGER/Line products are US
Government works in the **public domain** — no license obligation — but for
consistency with the OSM precedent we attribute them. Surfaced notice:

> Community boundaries &copy; US Census Bureau (TIGER/Line, public domain).

The crime DATA itself is official Montgomery County PD / Prince George's County
PD open data; only the place geometry is Census-derived.
